 ...
 * @description 广告滑动页的第一个页面
 */
public class PageOneFragment extends BaseFragment implements OnClickListener {
	...
	protected void initialized() {
		if (ValueUtil.isListEmpty(alleylists)) {
			...
			content.setText("      "+alleylists.get(1).getContent());
			ImageCacheUitl.displayNormalImageView(pagerview, alleylists.get(1)
					.getImg(), R.id.pagerview);
		}
	}
		...
		Api.GetAlley(new ApiCallBack() {
			...
			public void succeed(Result result) {
				if (result.isValidate()) {
					...
					content.setText(alleylists.get(1).getContent());
					ImageCacheUitl.displayNormalImageView(pagerview, alleylists
							.get(1).getImg(), R.id.pagerview);
				}
			}
